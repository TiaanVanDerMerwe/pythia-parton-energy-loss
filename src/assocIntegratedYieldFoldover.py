...
# ─────────────────────────────────────────────────────────────────────────────
from projectionsFoldoverOG import (
...
def compute_variant_ratio(res_q, res_v, near_limit=NEAR_SIDE_LIMIT):
    ...
    Yn_q, Ya_q = _integrate_one(res_q['phi_proj'], phi_c, near_mask, away_mask, bw, dpt_q)
    Yn_v, Ya_v = _integrate_one(res_v['phi_proj'], phi_c, near_mask, away_mask, bw, dpt_v)
    ...
    if len(jk_q) != len(jk_v):
        print("  WARNING: quenched/vacuum jackknife block counts differ "
    ...
    factor = (n_blocks - 1) / n_blocks
    sigma_near = float(np.sqrt(factor * np.nansum((r_near_k - np.nanmean(r_near_k)) ** 2)))
    sigma_away = float(np.sqrt(factor * np.nansum((r_away_k - np.nanmean(r_away_k)) ** 2)))
...
def plot_dphi_slices(results_by_variant, trig_pt_range=(TRIG_PT_MIN, TRIG_PT_MAX),
    ...
    if len(res_q) != len(res_v):
        print("  WARNING: quenched/vacuum have different numbers of valid "
    ...
    for i in range(n_slices):
        ...
        for variant, res in (('vacuum', res_v[i]), ('quenched', res_q[i])):
            ...
            ax.step(phi, sig, where='mid', color=color, linestyle="-", linewidth=2,
                    label=f'{VARIANT_LABELS[variant]}  ZYAM={res["background"]:.4f}',
                    zorder=4)
            ax.errorbar(phi, sig, yerr=err, fmt='none', ls='none',
                        color=color, ms=5, lw=1.4, zorder=4)
    ...
    fig.suptitle(
        rf'CMS Experimental Dihadron $\Delta\phi$ Distributions'+'\n'
...
def plot_integrated_yields(results_by_variant, trig_pt_range=(TRIG_PT_MIN, TRIG_PT_MAX),
                           cms_dir='datathief', save_path=None,
    ...
    for name, res_list in (('quenched', res_q), ('vacuum', res_v)):
        ...
        for r in res_list:
            Yn, Ya, sn, sa = integrate_yield(
                r['phi_centers'], r['phi_proj'], r['jackknife_projs'],
                assoc_pt_range=r['assoc_pt_range'],
            )
            yn.append(Yn); ya.append(Ya); en.append(sn); ea.append(sa)
        variant_yields[name] = {
    ...
    for rq, rv in zip(res_q, res_v):
        ...
        ratio_near.append(Rn); ratio_away.append(Ra)
        ratio_near_err.append(sn); ratio_away_err.append(sa)
    ...
    sides = [
        ('near', False, 'Near side', r'$|\Delta\phi| < 1$ rad', ratio_near, ratio_near_err),
        ('away', True,  'Away side', r'$1 < |\Delta\phi| < \pi$ rad', ratio_away, ratio_away_err),
    ]

    for col, (side_key, away, label, phi_label, r_ratio, r_ratio_err) in enumerate(sides):
        ...
        if cms_pt is not None and len(cms_pt) == len(pt_centers):
            ...
            ax_cms_rat.errorbar(pt_centers, cms_ratio, xerr=xerr, yerr=cms_ratio_err,
                                fmt='o', color=base_color, ms=6, markeredgewidth=0.6,
                                ecolor=base_color, elinewidth=1.5, capsize=4, zorder=4)
        else:
            ax_cms_rat.text(0.5, 0.5, 'CMS data not loaded',
                            ha='center', va='center',
                            transform=ax_cms_rat.transAxes, fontsize=9, color='gray')
        ...
        ax_qv_rat.errorbar(pt_centers, r_ratio, xerr=xerr, yerr=r_ratio_err,
                           fmt='o', color='#333333', ms=6, markeredgewidth=0.6,
                           ecolor='#333333', elinewidth=1.5, capsize=4, zorder=4)
...
def save_yield_csv(slice_results, trig_pt_range, out_dir, variant,
                   n_blocks=N_JACKKNIFE_BLOCKS):
    """
    ...
    with open(save_path, 'w') as f:
        ...
        f.write('#\n')
        f.write('assoc_pt_lo,assoc_pt_hi,assoc_pt_center,Y_near,err_near,Y_away,err_away\n')
...
def save_ratio_csv(res_q, res_v, trig_pt_range, out_dir):
    ...
    with open(save_path, 'w') as f:
        ...
        f.write('#\n')
        f.write('assoc_pt_lo,assoc_pt_hi,assoc_pt_center,R_near,err_near,R_away,err_away\n')
        for i in range(n):
            rq, rv = res_q[i], res_v[i]
            alo, ahi = rv['assoc_pt_range']
            Rn, Ra, sn, sa = compute_variant_ratio(rq, rv)
...
def main():
    ...
    plot_dphi_slices(
        results, trig_pt_range=trig_range, cms_dir='datathief',
